...
本项目为开源学习项目，旨在探索和学习 Electron、Vue 3 及多平台打包技术。

**郑重声明**：
1. 本项目**不提供**任何影视视频的存储、服务器或上传服务。
2. 项目内部使用的所有数据和视频源均来自互联网公开的第三方 API 接口，与本项目及开发者无关。
3. 本项目仅供私下技术交流、学习和研究使用，下载后请在 24 小时内自行删除。
4. 任何涉及商业盈利目的的单位或个人，均不得随意传播或使用本项目，否则产生的一切法律后果由使用者自行承担，与本项目及开发者无关。
5. 若在使用过程中发现任何侵犯您权益的第三方资源，请直接联系对应资源的原始提供方（数据源网站）进行处理。
